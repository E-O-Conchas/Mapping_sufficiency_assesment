...
gpkg_file_2018_path <- "I:/biocon/Emmanuel_Oceguera/projects/2023_03_ETC_BE/Task 1.1.7.2 Protected areas dataflows/Subtask 2.viii Sufficiency assesment/outputs/PL/Natura_2000_sites_v2018/species/natura_2000_PL_species_all_sites_ETRS89.gpkg"
gpkg_file_2023_path <- "I:/biocon/Emmanuel_Oceguera/projects/2023_03_ETC_BE/Task 1.1.7.2 Protected areas dataflows/Subtask 2.viii Sufficiency assesment/outputs/PL/Natura_2000_sites_v2023/species/natura_2000_PL_species_all_sites_ETRS89.gpkg"
output_file <- "Changes_Natura2000_sites_v2018_v2023/species"
...
natura2000_spe_2018 <- st_read(gpkg_file_2018_path, layer = layer_name) # every time when you run the function set the layers name
natura2000_spe_2023 <- st_read(gpkg_file_2023_path, layer = layer_name) # every time when you run the function set the layers name

# # Convert to data frames
# natura2000_spe_2018_df <- as.data.frame(natura2000_spe_2018)
# natura2000_spe_2023_df <- as.data.frame(natura2000_spe_2023)

# Reduce datasets to necessary columns only
spe_n2k_2018 <- natura2000_spe_2018 %>% select(SITECODE, SPECIESCODE, SPECIESNAME, POPULATION)
spe_n2k_2023 <- natura2000_spe_2023 %>% select(SITECODE, SPECIESCODE, SPECIESNAME, POPULATION)
...
remaining_in_site <- spe_n2k_2023 %>%
  ...
  filter(POPULATION.2023 != "D" & POPULATION.2018 != "D") %>%
...
added_to_existing_site <- spe_n2k_2023 %>%
  left_join(spe_n2k_2018, by = c("SITECODE", "SPECIESCODE"), suffix = c(".2023", ".2018"), relationship = "many-to-many") %>%
  filter((is.na(POPULATION.2018) & POPULATION.2023 != "D") |
           (POPULATION.2018 == "D" & !POPULATION.2023 %in% c("D"))) %>%
...
added_to_a_new_site <-  spe_n2k_2023 %>% 
  anti_join(spe_n2k_2018, by = "SITECODE") %>% 
  semi_join(spe_n2k_2023, by = "SPECIESCODE") %>% 
  filter(POPULATION %in% c("A", "B", "C", "")) %>%
  mutate(CATEGORY = "Added to a new site") %>%
  rename(POPULATION.2023 = POPULATION)

# added_to_a_new_site <- spe_n2k_2023 %>%
#   anti_join(spe_n2k_2018, by = c("SITECODE", "SPECIESCODE")) %>%
#   filter(POPULATION %in% c("A", "B", "C", "")) %>%
#   mutate(CATEGORY = "Added to a new site")%>%
#   rename(POPULATION.2023 = POPULATION)

names(added_to_a_new_site)
...
deleted_from_site <- spe_n2k_2018 %>%
  ...
  rename(POPULATION.2018 = POPULATION)

names(deleted_from_site)
...
population_decline <- spe_n2k_2018 %>%
  ...
  filter(POPULATION.2018 %in% c("A", "B", "C", "") & POPULATION.2023 == "D") %>%
  mutate(CATEGORY = "Population declined to D")

names(population_decline)
